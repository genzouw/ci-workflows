...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/ci-workflows`](https://github.com/genzouw/ci-workflows) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
上記をひとつでも満たさないサービスを組み込む PR は **MUST NOT** で、提出された場合は内容の良し悪しにかかわらずオーナーのレビューでクローズされます。

### 1.1 MUST NOT — これらを含む PR はオーナーのレビューでクローズされます
...
ここでいう Issue は **本ポリシー (本ファイル) 自体の改定を提案するもの** であり、個別の有料サービス導入 PR を承認するものではありません。1 および 1.1 の MUST NOT とクローズ判断は、リポジトリオーナー (@genzouw) が Issue 上で同意を示した場合でも適用され続けます。本ファイルの該当条項が実際に改定されるまでは、有料サービスを導入する PR を作成しないでください。
本ポリシーから外れる導入を検討したい場合は、PR を作成する **前に** Issue で提案し、リポジトリオーナー (@genzouw) の意見を **SHOULD** 求めてください。
Issue では「なぜ既存の無料サービスでは目的を達成できないか」「課金リスクをどう管理するか」「本ポリシーをどう改定すべきか」を明確に書いてください。
