...
ここでいう Issue は **本ポリシー (本ファイル) 自体の改定を提案するもの** であり、個別の有料サービス導入 PR を承認するものではありません。1 および 1.1 の MUST NOT と自動クローズは、リポジトリオーナー (@genzouw) が Issue 上で同意を示した場合でも適用され続けます。本ファイルの該当条項が実際に改定されるまでは、有料サービスを導入する PR を作成しないでください。
